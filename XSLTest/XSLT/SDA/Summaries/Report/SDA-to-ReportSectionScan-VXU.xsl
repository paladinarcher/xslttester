...
<xsl:template match="/">
	<xsl:element name="{$sectionName}">
		<xsl:apply-templates select="/Container/Patients/Patient/Encounters/Encounter/Medications/Medication">
...
<xsl:template match="Medication">
	<xsl:if test="DrugProduct/OrderType = 'VXU' or  OrderItem/OrderType = 'VXU'">
		...
		<xsl:if  test="string-length($subTypes)= 0 or contains(concat(',',$subTypes,','),concat(',',$orderCategory,','))">
			...
			<xsl:variable name="desc"> 
				<xsl:choose>
					<xsl:when test="DrugProduct/Description"><xsl:value-of select="DrugProduct/Description"/></xsl:when>
					<xsl:otherwise><xsl:value-of select="OrderItem/Description"/></xsl:otherwise>
				</xsl:choose>
			</xsl:variable>
