...
<xsl:template match="/Root/query:AdhocQueryResponse">
<xsl:apply-templates mode="RegistryObjectList" select="rim:RegistryObjectList"/>
</xsl:template>
...
<xsl:template match="*" mode="RegistryObjectList">
<Container>
...
<xsl:variable name="embeddedDoc" select="/Root/Documents/Document[@id=$docUniqueId][@repo=$repoUniqueId]"/>
<Document>
...
<xsl:choose>
<xsl:when test="$embeddedDoc/@mimeType!=''">
<FileType><xsl:value-of select="$embeddedDoc/@mimeType"/></FileType>
</xsl:when>
<xsl:otherwise>
...
<xsl:choose>
<xsl:when test="$embeddedDoc/text()!=''">
<Stream><xsl:value-of select="$embeddedDoc/text()"/></Stream>
</xsl:when>
<xsl:otherwise>
...
<DocumentURL><xsl:value-of select="isc:evaluate('makeURL','HS.UI.IHE.DocumentViewer.cls','FAC',$facility,'AA',$assigningAuthority,'MRN',$patientId,'UUID',$docUUID,'HOME',$docHOME)"/></DocumentURL>
</xsl:otherwise>
</xsl:choose>
