...
<xsl:stylesheet
	...
	<xsl:param name="thruDate"/>
	
	<xsl:variable name="_start" select="isc:evaluate('dateDiff','dd',translate($fromDate,'TZ',' '))"/>
	<xsl:variable name="_end" select="isc:evaluate('dateDiff','dd',translate($thruDate,'TZ',' '))"/>
