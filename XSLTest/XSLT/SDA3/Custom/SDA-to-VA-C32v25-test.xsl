...
<!--  TODO: confirm ALLERGY CODE construct & confirm where OriginalTextAllergyCategory is valued after translation -->
			...
			********************************************************
			...
			<component> <xsl:comment> Component 2 </xsl:comment>
				<section>
        ...
        <xsl:choose><!-- TODO Meds Filtering [((OrderCategory/Code/text() = 'O RX' or OrderCategory/Code/text() = 'O') and isc:evaluate('dateDiff', 'mm', translate((Extension/LastFilled | Extension/Expires)/text(), 'TZ', ' ')) &lt; 16 and not(Status/text() = 'DELETED')) or (OrderCategory/Code/text() = 'NV RX' and not(Status/text() = 'DISCONTINUED'))]-->
          ...
          <xsl:otherwise>
			...
			********************************************************
			...
			<component> <xsl:comment> Component 4 </xsl:comment>
				<xsl:choose>
					...
					<xsl:otherwise>
						<section>
													...
													<list>
														<item>
															<content ID="{concat('vndMeasurement', position())}">
																<xsl:value-of select="ObservationCode/Description" /> <xsl:value-of select="Observation/ObservationValue" /> <xsl:value-of select="ObservationCode/ObservationValueUnits/Code" />
															</content>
														</item>
													</list>
												</td>
							...
							<xsl:for-each select="Observations/Observation[generate-id() = generate-id(key('vitals', GroupId)[1])]">
								<xsl:sort select="ObservationTime" order="descending" />
								<xsl:variable name="grp" select="GroupId/text()" />
								<entry typeCode="DRIV"> <xsl:comment> Repeat Entry/Organizer block for each vital sign panel </xsl:comment>
									<organizer classCode="CLUSTER" moodCode="EVN">
										...
										<author>
											...
											<assignedAuthor>
												...
												<xsl:comment> HITSP C32 V2.5:   C83 author/assignedPerson/Name  Required but VA VistA data not yet available </xsl:comment>
												<assignedPerson>
														<name><xsl:value-of select="EnteredBy/Description"/></name>
												</assignedPerson>
												<representedOrganization>
													<xsl:comment> INFORMATION SOURCE FACILITY OID (ID = VA OID, EXT = TREATING FACILITY NBR) </xsl:comment>
													<id extension="" root="2.16.840.1.113883.4.349"/>
                      ...
                      <xsl:for-each select="Result/ResultItems/LabResultItem">
                        <tr >
                          ...
                          <td/>
							...
							<entry> <xsl:comment> Do Not Repeat per Result  </xsl:comment>
                            ...
                            <high value="{translate(SpecimenCollectedTime/text(),'TZ:- ','')}"/>
											...
											<xsl:for-each select="Result/ResultItems/LabResultItem">
												<component> <xsl:comment> Component Repeats for Each VA Test Result </xsl:comment>
													<observation classCode="OBS" moodCode="EVN">
                                    ...
                                    <reference value="{concat('#loincLabValues',$lid,'-',position())}"/>
														...
														<entryRelationship typeCode='SUBJ' inversionInd='true'>
														  <act classCode='ACT' moodCode='EVN'>
																...
																<text><reference value="{concat('#lndComment',position())}"/></text>
																<statusCode code='completed' />
																<author>
														  		<time nullFlavor="UNK"/>
																  <assignedAuthor>
...
<hkh></hkh>
				<component> <xsl:comment> Component 6 </xsl:comment>
					<section>
						<xsl:comment> Immunizations section template </xsl:comment>
						<templateId root="2.16.840.1.113883.10.20.1.6"/>                                                 <templateId root="1.3.6.1.4.1.19376.1.5.3.1.3.23"/>
						...
						<entry typeCode="DRIV"> <xsl:comment> Repeat entry/substanceAdminstration block for each Immunization </xsl:comment>
							...
							<substanceAdministration classCode="SBADM" moodCode="EVN" negationInd="false">
								...
								<xsl:comment> COMMENT REFERENCE points to Narrative Block </xsl:comment>
								<text><reference value="#indComments"/></text>
								<statusCode code="completed"/>
                    ...
                    <!-- <manufacturedLabeledDrug></manufacturedLabeledDrug> -->
								...
								<entryRelationship typeCode='CAUS' inversionInd="false">  <xsl:comment> Repeat entryRelationship/observation block for each immuniz reaction </xsl:comment>
									<observation classCode='OBS' moodCode='EVN'>
										<templateId root='2.16.840.1.113883.10.20.1.28'/>
										<templateId root='1.3.6.1.4.1.19376.1.5.3.1.4.5'/>
			...
			********************************************************
			...
			<component> <xsl:comment> Component 7 </xsl:comment>
				<section>
				<xsl:comment>CCD Encounters Section Conformance Identifier</xsl:comment>
				  ...
				  <entry> <xsl:comment> Entry block repeats for each Encounter </xsl:comment>
	     ...
	     <xsl:comment> 
			...
			********************************************************
			...
			<component> <xsl:comment> Component 8 </xsl:comment>
				<section>
				<xsl:comment> Procedures section template </xsl:comment>
					...
					<entry typeCode="DRIV"> <xsl:comment> HITSP C32 V2.5:  Entry block Repeats for Each Procedure </xsl:comment>
						<procedure classCode="PROC" moodCode="EVN">
							...
							<code code="procCode" displayName="procDisplay" codeSystem="2.16.840.1.113883.6.12" codeSystemName="CPT-4">
								...
								<qualifier> <xsl:comment> qualifier block repeats for each CPT Modifier </xsl:comment>
      ...
      <xsl:comment> 3.02 CONTACT TYPE, REQUIRED, classCode value determined by VistA value in contactType </xsl:comment>
        <associatedEntity classCode="{ContactType/Code/text()}">
          ...
          <code codeSystem='2.16.840.1.113883.5.111' codeSystemName='{isc:evaluate("getCodeForOID","2.16.840.1.113883.5.111","","2.16.840.1.113883.5.111")}' nullFlavor='NA'>
            <originalText nullFlavor="NA"><xsl:value-of select='Relationship/Code/text()' /></originalText>
          </code>
