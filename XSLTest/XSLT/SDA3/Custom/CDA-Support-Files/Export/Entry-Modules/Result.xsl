...
<xsl:template match="*" mode="labdiagnosticResults-NarrativeDetail">
	...
	<tr ID="{concat($exportConfiguration/*[local-name() = $narrativeLinkCategory]/narrativeLinkPrefixes/resultNarrative/text(), $narrativeLinkSuffix)}">
		<td ID="{concat('rsltDateTime-', position())}"><xsl:apply-templates select="ResultTime" mode="formatDateTime"/><xsl:apply-templates select="ResultTime" mode="formatTime"/></td>
			<td ID="{concat('rsltType-', position())}"><xsl:value-of select="EnteredAt/Description/text()"/></td>		
			 <td ID="{concat('rsltSource-', position())}"><xsl:apply-templates select="../OrderItem" mode="originalTextOrDescriptionOrCode"/></td>
			...
			<!--
			<td ID="{concat('rsltResultType-', position())}"><xsl:value-of select="ResultText/text()"/></td>
			-->

			<td ID="{concat('rsltResult-Unit-', position())}">
                                <!--<xsl:choose>
                                        <xsl:when test="string-length(ResultItems/LabResultItem/ResultValueUnits/text())">
                                                <xsl:value-of select="concat(ResultItems/LabResultItem/ResultValue/text(), ' ', ResultsItem/LabResultItem/ResultValueUnits/text())"/>
                                        </xsl:when>
                                        <xsl:otherwise>
                                                <xsl:value-of select="ResultItems/LabResultItem/ResultValue/text()"/>
                                        </xsl:otherwise>
                                </xsl:choose>-->
				<xsl:value-of select="ResultItems/LabResultItem/ResultValue/text()"/><xsl:text> </xsl:text><xsl:value-of select="ResultItems/LabResultItem/ResultValueUnits/text()"/>
                        </td>
  			<td ID="{concat('rsltInterpretation-', position())}"><xsl:value-of select="ResultItems/LabResultItem/ResultInterpretation/text()"/></td>
			<td ID="{concat('rsltReferenceRange-', position())}"><xsl:value-of select="ResultItems/LabResultItem/ResultNormalRange/text()"/></td>

			<td ID="{concat('rsltStatus-', position())}">
				<xsl:choose>
	                              <xsl:when test="contains('|F|FIN|','ResultStatus')">completed</xsl:when>
        	                      <xsl:when test="contains('|R|ENT|I|V|O|S|A|P|','ResultStatus')">active</xsl:when>
                	              <xsl:when test="contains('|C|COR|K|','ResultStatus')">completed</xsl:when>
                        	      <xsl:when test="'ResultStatus' = 'X'">cancelled</xsl:when>
	...
	<xsl:template match="*" mode="diagnosticResults-NarrativeDetail-LabResultItem">
		...
		<tr>
			...
			<td ID="{concat('andInterpretation-', position())}"><xsl:value-of select="ResultInterpretation/text()"/></td>
		...
		<tr ID="{concat($exportConfiguration/*[local-name() = $narrativeLinkCategory]/narrativeLinkPrefixes/resultNarrative/text(), $narrativeLinkSuffix)}">
			...
			<xsl:choose>
				<xsl:when test="DocumentType/Code='LR'">
					<td ID="{concat('andLOINC-', position())}">LR SURGICAL PATHOLOGY REPORT (27898-6)</td>
				</xsl:when>
				<xsl:otherwise>
					<td ID="{concat('andLOINC-', position())}"><xsl:value-of select="DocumentName/text()"/> (18726-0)</td>
				</xsl:otherwise>
			</xsl:choose>
			<td ID="{concat('andSource-', position())}"><xsl:value-of select="EnteredAt/Description/text()"/></td>
			<td ID="{concat('andResult-', position())}"><xsl:value-of select="NoteText/text()"/></td>
			<td ID="{concat('andProvider-', position())}"><xsl:value-of select="EnteredBy/Description"/></td>
			<td ID="{concat('andStatus-', position())}"><xsl:value-of select="DocumentCompletionStatus/Description/text()"/></td>
