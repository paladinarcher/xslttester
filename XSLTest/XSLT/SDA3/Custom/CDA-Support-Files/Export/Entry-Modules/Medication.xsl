	...
	<xsl:template match="*" mode="medications-NarrativeDetail">
		...
		<xsl:if test="($includeInExport = 1)">
			...
			<tr ID="{concat($exportConfiguration/*[local-name() = $narrativeLinkCategory]/narrativeLinkPrefixes/medicationNarrative/text(), $narrativeLinkSuffix)}">
				<td ID="{concat('mndMedication-', position())}"><xsl:apply-templates select="DrugProduct" mode="originalTextOrDescriptionOrCode"/></td>
				<!-- add status here -->
                                <td ID="{concat('mndStatus-', position())}">
				<xsl:choose>
                                        <xsl:when test="Status = 'A'">Active</xsl:when>
                                        <xsl:when test="Status = 'E'">Expired</xsl:when>
                                        <xsl:when test="Status = 'I'">Inactive</xsl:when>
                                        <xsl:when test="Status = 'R'">Resolved</xsl:when>
                                        <xsl:otherwise>--</xsl:otherwise>
                                </xsl:choose>
				</td>
				
				<!-- add quantity here -->
				<td ID="{concat('mndQuantity-', position())}"><xsl:value-of select="concat(DosageSteps/DosageStep/DoseQuantity/text(), DosageSteps/DosageStep/DoseQuantity/DoseUoM/Code/text())"/></td>

				<!-- Exp time -->
				<td ID="{concat('mndOrderExpiration-', position())}"><xsl:apply-templates select="ToTime" mode="formatDateTime"/></td>

				<!-- Provider -->
				<td ID="{concat('mndProvider-', position())}"><xsl:apply-templates select="OrderedBy" mode="name-Person-Narrative"/></td>

				<!-- Prescription # -->
				<td ID="{concat('mndPrescription-', position())}"><xsl:value-of select="PrescriptionNumber"/></td>

				<!-- Dispense Date -->
				<td ID="{concat('mndDispenseDate-', position())}"><xsl:apply-templates select="FromTime" mode="formatDateTime"/></td>

				<!-- Sig -->
				 <td ID="{concat('mndSig-', position())}"><xsl:value-of select="Extension/Sig"/></td>                                
                                <!-- Source -->
				<td ID="{concat('mndSource-', position())}"><xsl:value-of select="EnteredAt/Description"/></td>
