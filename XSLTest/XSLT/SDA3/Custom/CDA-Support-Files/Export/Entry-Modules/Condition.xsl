	...
	<xsl:template match="*" mode="conditions-NarrativeDetail">
		...
		<xsl:if test="($includeInExport = 1)">
			...
			<tr ID="{concat($exportConfiguration/*[local-name() = $narrativeLinkCategory]/narrativeLinkPrefixes/conditionNarrative/text(), $narrativeLinkSuffix)}">
				<td ID="{concat('pndProblem-', position())}"><xsl:value-of select="ProblemDetails"/></td>
				<td ID="{concat('pndStatus-', position())}"><xsl:value-of select="Status/Description/text()"/></td>
				<td ID="{concat('pndProblemCode-', position())}"><xsl:value-of select="Problem/Code/text()"/></td>
				<td ID="{concat('pndDateOfOnset-', position())}"><xsl:apply-templates select="UpdatedOn" mode="narrativeDateFromODBC"/></td>
				<td ID="{concat('pndProvider-', position())}"><xsl:apply-templates select="Clinician" mode="name-Person-Narrative"/></td>
				<td ID="{concat('pndDateOfOnset-', position())}"><xsl:value-of select="EnteredAt/Description/text()"/></td>
			</tr>
		</xsl:if>
	</xsl:template>
