...
<xsl:stylesheet version="1.0" xmlns="urn:hl7-org:v3" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:sdtc="urn:hl7-org:sdtc" xmlns:isc="http://extension-functions.intersystems.com" xmlns:exsl="http://exslt.org/common" xmlns:set="http://exslt.org/sets" exclude-result-prefixes="isc xsi sdtc exsl set">
		...
		<text>
			...
			<xsl:choose>
                ...
                <xsl:otherwise>
                  ...
                  date range. If no date range was provided, it shows the 3 most recent assessment scores that were 
                  completed within the last 3 years. Data comes from all VA treatment facilities. 
                </xsl:otherwise>
            </xsl:choose>
       		</paragraph>
       		<paragraph>
			...
			-->
				...
				<tbody>
					<xsl:apply-templates select="Problems/Problem[Category/Code='248536006' or Problem/Code='408907016']" mode="functionalStatus-NarrativeDetail"/>
				</tbody>
			</table>
		</text>
	</xsl:template>
	...
	<xsl:template match="*" mode="functionalStatus-NarrativeDetail">
		...
		<tr ID="{concat($exportConfiguration/functionalStatus/narrativeLinkPrefixes/functionalStatusNarrative/text(), $narrativeLinkSuffix)}">
			...
			<xsl:otherwise> 
				...
				<td ID="{concat('functionalStatusDetails-', $narrativeLinkSuffix, '-', position())}"><xsl:call-template name="functionalStatus-Detail"/></td>
				</tr>
			</xsl:otherwise>
				</xsl:choose>
				<xsl:if test="contains(Name/text(),'Score')">
	...
	<xsl:template name="functionalStatus-Detail">
		<xsl:choose>
