...
<xsl:stylesheet version="1.0" xmlns="urn:hl7-org:v3" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:sdtc="urn:hl7-org:sdtc" xmlns:isc="http://extension-functions.intersystems.com" xmlns:exsl="http://exslt.org/common" xmlns:set="http://exslt.org/sets" exclude-result-prefixes="isc xsi sdtc exsl set">
	...
	<xsl:template match="*" mode="diagnosticResults-NarrativeDetail">
		...
		<tr ID="{concat($exportConfiguration/*[local-name() = $narrativeLinkCategory]/narrativeLinkPrefixes/resultNarrative/text(), $narrativeLinkSuffix)}">
			...
			<td><xsl:value-of select="Extension/Description/text()"/></td>
			<td><xsl:value-of select="LabResultItem/PerformedAt/Description/text()"/></td>
			<td><xsl:value-of select="LabResultItem/ResultValueUnits"/></td>
			<td><xsl:value-of select="ResultInterpretation"/></td>
			<td><xsl:value-of select="LabResultItem/ResultNormalRange"/></td>
			<td><xsl:value-of select="ResultStatus"/></td>
		
			<!--
			<td ID="{concat($exportConfiguration/*[local-name() = $narrativeLinkCategory]/narrativeLinkPrefixes/resultTestDescription/text(), $narrativeLinkSuffix)}"><xsl:apply-templates select="../OrderItem" mode="originalTextOrDescriptionOrCode2"/></td>
			<td><xsl:apply-templates select="ResultTime" mode="narrativeDateFromODBC"/></td>
	...
	<xsl:template match="*" mode="diagnosticResults-NarrativeDetail-LabResultItem">
		...
		<xsl:variable name="testItemDescription"><xsl:apply-templates select="TestItemCode" mode="originalTextOrDescriptionOrCode2"/></xsl:variable>
		<tr>
			<td></td>
			<!--<td><xsl:value-of select="translate(concat($testItemDescription, ' (test code = ', TestItemCode/Code/text(), ')'),'&amp;','_')"/></td>-->
