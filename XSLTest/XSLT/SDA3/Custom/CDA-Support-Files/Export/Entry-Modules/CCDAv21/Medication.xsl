  ...
  <!-- AlsoInclude: AuthorParticipation.xsl Comment.xsl -->
	...
	<xsl:template match="*" mode="eM-medications-Narrative">
		...
		<text>
			...
			<paragraph>
				<content ID="medTime">Section Date Range: From patient's date of birth to the date document was created.</content>
			</paragraph>
			<paragraph>
				This section includes: 1) prescriptions processed by a VA pharmacy in the last 15 months, and 2) all medications recorded in the VA medical record as "non-VA medications". Pharmacy terms refer to VA pharmacy's work on prescriptions. VA patients are adviced to take their medications as instructed by their health care team. The data comes from all VA treatment facilities.
			</paragraph>
			<paragraph>
			...
			<tr ID="{concat($exportConfiguration/*[local-name() = $narrativeLinkCategory]/narrativeLinkPrefixes/medicationNarrative/text(), $narrativeLinkSuffix)}">
					...
					<xsl:choose>
						<xsl:when test="contains(OrderCategory/Description,'NON-VA MEDICATIONS')">
							...
							<td>Non-VA</td>
							<td></td>
							<td>Documented by:  <xsl:apply-templates select="OrderedBy" mode="fn-name-Person-Narrative"/></td>
						...
						<xsl:otherwise>
							...
							<td>
							<xsl:choose>
								<xsl:when test="$isCurrentMedication = 1">Current</xsl:when>
