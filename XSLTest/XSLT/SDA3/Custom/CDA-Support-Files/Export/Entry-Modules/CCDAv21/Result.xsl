  ...
  <!-- AlsoInclude: AuthorParticipation.xsl Comment.xsl -->
	...
	<xsl:template match="*" mode="eR-diagnosticResults-Narrative">
		...
		<!-- VA Results Business Rules for Medical Content -->
     		...
     		<xsl:choose>
     			<xsl:when test="$flavor = 'SES'">
        			This section includes the Chemistry and Hematology Lab Results, Radiology Reports, and 
        			Pathology Reports on record with VA for the patient. 
        		</xsl:when>
 				<xsl:when test="$flavor = 'MHV'">
                	This section contains the Chemistry and Hematology Lab Results, Radiology Reports, and 
                    	Pathology Reports on record with VA for the patient. The data comes from all VA treatment 
                    	facilities.	
                </xsl:when>
               	<xsl:otherwise>
                    	This section contains the Chemistry and Hematology Lab Results, Radiology Reports, and 
                    	Pathology Reports on record with VA for the patient within the requested date range. If no 
                    	date range was provided, the lists include data from the last 24 months. The data comes from 
			...
			<paragraph>
				<xsl:choose>
					<xsl:when test="$flavor = 'SES'">
        				This section contains the Chemistry/Hematology Results that were resulted 30 days 
        				before or 30 days after the date of the Encounter. 
        			</xsl:when>
                	<xsl:when test="$flavor = 'MHV'">
                		The included Chemistry/Hematology Results are from the last 24 months, are available 3 
                		calendar days after verification, and include a maximum of the 10 most recent sets of 
                		tests. The data comes from all VA treatment facilities. 
                	</xsl:when>
                	<xsl:otherwise>
                    	...
                    	from the last 24 months and are limited to the 10 most recent sets of tests. The data 
			...
			<table border="1" width="100%">
				...
				<tbody>
					<xsl:apply-templates select="LabOrders/LabOrder/Result[string-length(ResultText/text()) or string-length(ResultItems)and not(ResultType/text()='SP')]" mode="eR-diagnosticResults-NarrativeDetail">
		...
		<xsl:call-template name="documents-Narrative">
			<xsl:with-param name="documentTypes" select="'Radiology Reports'"/>
			<xsl:with-param name="documentType" select="'Radiology Report'"/>
			<xsl:with-param name="documentText1" select="'This section includes all Radiology Reports'"/>
			<xsl:with-param name="documentText2" select="'included Radiology Reports are'"/>
			<xsl:with-param name="documentText3" select="'reports'"/>
			<xsl:with-param name="documentCode" select="'RA'"/>
			<xsl:with-param name="documentN" select="'5'"/>
			<xsl:with-param name="documentD" select="'3'"/>
			<xsl:with-param name="documentM" select="'24'"/>
			<xsl:with-param name="documentTitles" select="''"/>
		</xsl:call-template>
		
		<xsl:call-template name="documents-Narrative">
			<xsl:with-param name="documentTypes" select="'Pathology Reports'"/>
			<xsl:with-param name="documentType" select="'Pathology Report'"/>
			<xsl:with-param name="documentText1" select="'This section includes all Pathology Reports'"/>
			<xsl:with-param name="documentText2" select="'included Pathology Reports are'"/>
			<xsl:with-param name="documentText3" select="'reports'"/>
			<xsl:with-param name="documentCode" select="'LR'"/>
			<xsl:with-param name="documentN" select="'5'"/>
			<xsl:with-param name="documentD" select="'14'"/>
			<xsl:with-param name="documentM" select="'24'"/>
			<xsl:with-param name="documentTitles" select="''"/>
		</xsl:call-template>
	</text>	
