...
<xsl:stylesheet version="1.0" xmlns="urn:hl7-org:v3" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:sdtc="urn:hl7-org:sdtc" xmlns:isc="http://extension-functions.intersystems.com" xmlns:exsl="http://exslt.org/common" xmlns:set="http://exslt.org/sets" exclude-result-prefixes="isc xsi sdtc exsl set">
    ...
    <xsl:variable name="hasCareTeamFolks" select="boolean(/Container/Patient/Extension/CareTeamMembers/CareTeamMember)"/>
		...
		<xsl:choose>
            ...
            <!-- Condition Physicians - Care Team Folks -->
						<xsl:if test="$hasCareTeamFolks">
							<xsl:apply-templates select="Extension/CareTeamMembers" mode="eHP-documentationOf-CareTeamFolks"/>
						</xsl:if>
  ...
  <xsl:template match="CareTeamMembers" mode="eHP-documentationOf-CareTeamFolks">
    <xsl:for-each select="set:distinct(CareTeamMember/Code)">
      <xsl:variable name="careProviderCode" select="text()"/>

      <performer typeCode="PRF">
        <templateId extension="2014-06-09" root="2.16.840.1.113883.10.20.6.2.1" />
        <!-- <xsl:apply-templates select="../CareProviderType" mode="fn-code-function"/> -->
        ...
        <xsl:apply-templates select=".." mode="eHP-assignedEntity-careteamfolks"/>
      </performer>
    </xsl:for-each>
  </xsl:template>
  <xsl:template match="*" mode="eHP-assignedEntity-careteamfolks">
    ...
    <assignedEntity classCode="ASSIGNED">
      ...
      <id nullFlavor="NI" />
      <code code="{CareProviderType/Code/text()}" codeSystem="" codeSystemName="" displayName="{CareProviderType/Extension/Classification/text()}" />
