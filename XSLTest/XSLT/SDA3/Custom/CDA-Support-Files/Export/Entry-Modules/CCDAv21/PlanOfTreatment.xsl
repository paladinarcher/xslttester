	...
	<xsl:template match="*" mode="ePOT-planOfTreatment-Narrative">
		...
		<text>
			...
			<xsl:choose>
        		...
        		The Plan of Treatment section includes future care activities for the patient from all VA 
        		treatment facilities. This section includes future appointments and future orders which 
        		are active, pending or scheduled. 	
        		</xsl:when>
        		<xsl:otherwise>
 				The Plan of Treatment section contains future care activities for the patient from all VA treatment 
 				facilities. This section includes future appointments (within the next 6 months) and future orders 
 				(within +/- 45 days) which are active, pending or scheduled.
 				</xsl:otherwise>
 				</xsl:choose> 
        	</paragraph>
			<paragraph>
			...
			<table border="1" width="100%">
				...
				<tbody>
				<tr>
				<td>This section includes appointments that were scheduled to occur 6 months from the 
				date of the Encounter, up to a maximum of 20 appointments. The data comes from all VA 
				treatment facilities.</td> 
				</tr>
				</tbody>
				</table>
			</xsl:when>
			<xsl:otherwise>
			<paragraph>
				This section includes up to a maximum of 20 appointments scheduled over the next 6 months. Some types 
				of appointments may not be included. Contact the VA health care team if there are questions. 
			</paragraph>
			</xsl:otherwise>
			</xsl:choose>
			...
			<table border="1" width="100%">
				...
				<tbody>
					<!-- Display appts 1st. For appointments, look at whether the pertinent date is in the future (i.e. dateDiff is less than zero). -->
			...
			<table border="1" width="100%">
				...
				<tbody>
				<tr>
					<td>This section includes a listing of several types of active, pending, and scheduled 
					orders, including clinic medications orders, diagnostic test orders, procedure orders and 
					consult orders; where the start date of the order is 45 days before the date of the 
					Encounter or 45 days after the date of the Encounter. The data comes from all VA treatment 
					facilities.</td>
				</tr>
				</tbody>
				</table>
			...
			<paragraph>
				This section includes a listing of several types of active, pending, and scheduled orders, including 
				clinic medication orders, diagnostic test orders, procedure orders, and consult orders; where the start date 
				of the order is 45 days before or after the date this document was created. 
			</paragraph>
			</xsl:otherwise>
			</xsl:choose>
			...
			<table border="1" width="100%">
				...
				<tbody>
					...
					<xsl:apply-templates select="RadOrders/RadOrder[Status/text()='IP']" mode="ePOT-planOfTreatment-NarrativeDetail"/>
					<xsl:apply-templates select="LabOrders/LabOrder[not(Result)]" mode="ePOT-planOfTreatment-NarrativeDetail"/>
					<xsl:apply-templates select="OtherOrders/OtherOrder[Status/text()='IP']" mode="ePOT-planOfTreatment-NarrativeDetail"/>
					<xsl:apply-templates select="Referrals/Referral[(Extension/Status/text()='PENDING') or isc:evaluate('dateDiff', 'dd', translate(ToTime/text(), 'TZ', ' ')) &lt; 0]" mode="ePOT-planOfTreatment-NarrativeDetail"/>
					<xsl:apply-templates select="Medications/Medication[isc:evaluate('dateDiff', 'dd', translate(FromTime/text(), 'TZ', ' ')) &lt; 0 ]" mode="ePOT-planOfTreatment-NarrativeDetail"/>
		...
		<xsl:variable name="activityTime">
			<xsl:choose>
				<xsl:when test="local-name()='LabOrder' or local-name()='RadOrder' or local-name()='OtherOrder'">
					<xsl:choose>
						<xsl:when test="string-length(SpecimenCollectedTime/text())">
							<xsl:apply-templates select="SpecimenCollectedTime" mode="formatDateTime"/>
						</xsl:when>
						<xsl:when test="string-length(FromTime/text())">
							<xsl:apply-templates select="FromTime" mode="formatDateTime"/>
						</xsl:when>
					</xsl:choose>
				</xsl:when>
				...
				<xsl:when test="local-name()='Procedure'">
					<xsl:choose>
						<xsl:when test="string-length(ProcedureTime/text())">
							<xsl:apply-templates select="ProcedureTime" mode="formatDateTime"/>
						</xsl:when>
						<xsl:when test="string-length(FromTime/text())">
							<xsl:apply-templates select="FromTime" mode="formatDateTime"/>
						</xsl:when>
				...
				<xsl:when test="local-name()='Goal'">
					<xsl:apply-templates select="ToTime" mode="formatDateTime"/>
				</xsl:when>
	...
	<xsl:template match="Referral" mode="ePOT-NarrativeDetail-ReferralDetails">
		...
		<xsl:variable name="referralAddress">
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:apply-templates select="ReferredToOrganization" mode="fn-descriptionOrCode"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
