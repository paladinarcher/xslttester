...
<xsl:stylesheet version="1.0" xmlns="urn:hl7-org:v3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="problemIntro">
	...
	<!-- VA Problem/Condition Business Rules for Medical Content -->
 	<paragraph ID="problemIntro">
            <xsl:choose>
				...
				This section includes a list of all active and inactive Problems/Conditions known to VA for the 
				patient. New problems/conditions are available 3 calendar days after entry. The data comes from all 
				VA treatment facilities. 
                </xsl:when>
                <xsl:otherwise>
				This section includes a list of Problems (Conditions) known to VA for the patient. It includes both 
				active and inactive problems (conditions). The data comes from all VA treatment facilities.
	...
	<xsl:template match="*" mode="eCn-conditions-Narrative">
		...
		<!-- VA Problem/Condition Business Rules for Medical Content -->
			...
			<table>
				...
				<tbody>
					<xsl:apply-templates select="Problems/Problem[not(Category/Code/text()='248536006') and not(Category/Code/text()='373930000')]" mode="eCn-conditions-NarrativeDetail">
	...
	<xsl:template match="Problem" mode="eCn-conditions-NarrativeDetail">
		...
		<xsl:variable name="includeInExport">
			...
			<tr ID="{concat($exportConfiguration/*[local-name() = $narrativeLinkCategory]/narrativeLinkPrefixes/conditionNarrative/text(), $narrativeLinkSuffix)}">
				...
				-->
				<td ID="{concat($exportConfiguration/*[local-name() = $narrativeLinkCategory]/narrativeLinkPrefixes/conditionStatus/text(), $narrativeLinkSuffix)}"><xsl:apply-templates select="Status" mode="fn-descriptionOrCode"/></td>
	...
	<xsl:template match="*" mode="eCn-conditions-Entries">
		...
		<xsl:apply-templates select="Problems/Problem[not(Category/Code/text()='248536006') and not(Category/Code/text()='373930000')]" mode="eCn-conditions-EntryDetail">
			<xsl:with-param name="narrativeLinkCategory" select="$narrativeLinkCategory"/>
			<xsl:with-param name="useCurrentConditions" select="$useCurrentConditions"/>
		</xsl:apply-templates>
	</xsl:template>
				...
				<act classCode="ACT" moodCode="EVN">
					...
					<!--
						...
						Note  : An SDA Problem is exported to the Problem List section
								...
								longer than the current condition time window.
					-->
					<xsl:apply-templates select="Extension/Comments/Comment/CommentText" mode="eCm-entryRelationship-comments">
