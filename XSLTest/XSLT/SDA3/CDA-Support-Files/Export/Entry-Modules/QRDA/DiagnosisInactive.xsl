...
<xsl:stylesheet version="1.0" xmlns="urn:hl7-org:v3" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:sdtc="urn:hl7-org:sdtc" xmlns:isc="http://extension-functions.intersystems.com" xmlns:exsl="http://exslt.org/common" xmlns:set="http://exslt.org/sets" exclude-result-prefixes="isc xsi sdtc exsl set">
	...
	<xsl:template match="Problem" mode="diagnosisInactive-Problem-Narrative">
		<xsl:param name="valueSetStringIn"/>
		
		<xsl:variable name="qualifies"><xsl:apply-templates select="." mode="diagnosisInactive-Problem-Qualifies"/></xsl:variable>
		<xsl:if test="$qualifies='1'">
			<!-- includeDiagnosisInExport template is in DiagnosisActive.xsl -->
			...
			<xsl:if test="($includeInExport = '1')">
				...
				<tr>
					<td>Diagnosis Inactive: <xsl:value-of select="Problem/Description/text()"/></td>
				...
				<xsl:if test="string-length(substring-after($valueSetString,'|'))">
					<xsl:apply-templates select="." mode="diagnosisInactive-Problem-Narrative">
	...
	<xsl:template match="Problem" mode="diagnosisInactive-Problem-Qualifies">1</xsl:template>
	
	<xsl:template match="Problem" mode="diagnosisInactive-Problem-Entry">
		<xsl:param name="valueSetStringIn"/>
		...
		<xsl:if test="$qualifies='1'">
			...
			<xsl:if test="($includeInExport = '1')">
				...
				<xsl:comment> QRDA Diagnosis Inactive </xsl:comment>
				<entry typeCode="DRIV">
					<observation classCode="OBS" moodCode="EVN">
						<templateId root="{$ccda-ProblemObservation}"/>
						<templateId root="{$qrda-DiagnosisInactive}"/>
						...
						<text>Diagnosis Inactive: <xsl:value-of select="Problem/Description/text()"/></text>
						...
						<!-- Problem Status -->
						<xsl:apply-templates select="." mode="observation-ProblemStatus-Inactive"/>
				...
				-->
				<xsl:if test="string-length(substring-after($valueSetString,'|'))">
					<xsl:apply-templates select="." mode="diagnosisInactive-Problem-Entry">
						<xsl:with-param name="valueSetStringIn" select="substring-after($valueSetString,'|')"/>
					</xsl:apply-templates>
				</xsl:if>
