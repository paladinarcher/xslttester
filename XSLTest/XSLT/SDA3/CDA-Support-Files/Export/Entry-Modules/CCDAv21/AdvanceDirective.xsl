	...
	<xsl:template match="AdvanceDirective" mode="eAD-advanceDirectives-NarrativeDetail">
		<tr ID="{concat($exportConfiguration/advanceDirectives/narrativeLinkPrefixes/advanceDirectiveNarrative/text(), position())}">
			<td ID="{concat($exportConfiguration/advanceDirectives/narrativeLinkPrefixes/advanceDirectiveType/text(), position())}">
				<xsl:choose>
					<xsl:when test="contains('|71388002|AD|', concat('|', AlertType/Code/text(), '|'))"><xsl:apply-templates select="Alert" mode="fn-descriptionOrCode"/></xsl:when>
					<xsl:otherwise><xsl:apply-templates select="AlertType" mode="fn-descriptionOrCode"/></xsl:otherwise>
				</xsl:choose>
			</td>
			<td>
				<xsl:choose>
					<xsl:when test="contains('|71388002|AD|', concat('|', AlertType/Code/text(), '|'))">N/A</xsl:when>
					<xsl:otherwise><xsl:apply-templates select="Alert" mode="fn-descriptionOrCode"/></xsl:otherwise>
				</xsl:choose>
			</td>
